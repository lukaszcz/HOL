...
[[ -z "$(find "$FINAL" -type l -print -quit)" ]]
...
TMP_CLEANUP="$FINAL/provenance/tmp-cleanup"
[[ -d "$TMP_CLEANUP" && ! -L "$TMP_CLEANUP" ]]
(cd "$TMP_CLEANUP" && sha256sum -c SHA256SUMS >/dev/null)
[[ "$(wc -l <"$TMP_CLEANUP/roots-before.tsv")" == 224 ]]
[[ ! -s "$TMP_CLEANUP/reference-scan.tsv" &&
   ! -s "$TMP_CLEANUP/roots-after.txt" ]]
cat "$TMP_CLEANUP/removed-first-attempt.tsv" \
  "$TMP_CLEANUP/removed-continuation.tsv" >"$TEMP/all-removed.tsv"
cmp "$TMP_CLEANUP/roots-before.tsv" "$TEMP/all-removed.tsv"
cmp "$TMP_CLEANUP/roots-before-continuation.tsv" \
  "$TMP_CLEANUP/removed-continuation.tsv"
[[ "$(awk -F '\t' '{b+=$4;f+=$5;d+=$6}
  END {printf "%d\t%d\t%d",b,f,d}' \
  "$TMP_CLEANUP/roots-before.tsv")" == $'10255043655\t19383\t2743' ]]
while IFS=$'\t' read -r target owner kind bytes files dirs extra; do
  [[ -n "$target" && -z "${extra-}" && "$owner" == 1003 &&
    "$kind" == directory && "$target" == /tmp/* &&
    "${target#/tmp/}" != */* && ! -e "$target" && ! -L "$target" ]]
  base=${target#/tmp/}
  [[ "$base" =~ [Pp][Hh][Aa][Ss][Ee]3|[Tt][Aa][Ss][Kk](10|11|12) ]]
done <"$TMP_CLEANUP/roots-before.tsv"
jq -e \
  --arg manifest "$(sha256sum "$TMP_CLEANUP/roots-before.tsv" |
    cut -d' ' -f1)" \
  --arg first "$(sha256sum "$TMP_CLEANUP/removed-first-attempt.tsv" |
    cut -d' ' -f1)" \
  --arg continuation "$(sha256sum \
    "$TMP_CLEANUP/removed-continuation.tsv" | cut -d' ' -f1)" \
  --arg empty "$(sha256sum "$TMP_CLEANUP/roots-after.txt" |
    cut -d' ' -f1)" \
  --arg references "$(sha256sum "$TMP_CLEANUP/reference-scan.tsv" |
    cut -d' ' -f1)" \
  --arg script "$(sha256sum "$TMP_CLEANUP/delete-explicit-roots.sh" |
    cut -d' ' -f1)" '
  .schema == "hh-task12-tmp-cleanup-v2" and .status == "complete" and
  .targets == 224 and .apparent_bytes == 10255043655 and
  .files == 19383 and .directories == 2743 and
  .roots_before_sha256 == $manifest and
  .dependency_validation.accepted_evidence_reference_matches == 0 and
  .dependency_validation.reference_scan_sha256 == $references and
  .dependency_validation.accepted_evidence_symlinks == 0 and
  .dependency_validation.sole_dependency_found == false and
  .dependency_validation.task10_a_inventory_verified == true and
  .dependency_validation.task10_certificate_semantics_verified == true and
  .dependency_validation.task11_frozen_verifier_passed == true and
  .dependency_validation.task12_frozen_verifier_passed == true and
  .deletion.path_validation_before_deletion == true and
  .deletion.direct_children_only == true and
  .deletion.symlink_targets_rejected == true and
  .deletion.first_attempt_removed == 41 and
  .deletion.first_attempt_manifest_sha256 == $first and
  .deletion.first_attempt_stopped_on_read_only_tree == true and
  .deletion.continuation_targets == 183 and
  .deletion.continuation_manifest_sha256 == $continuation and
  .deletion.continuation_removed_sha256 == $continuation and
  .deletion.safe_delete_script_sha256 == $script and
  .verification.matching_roots_remaining == 0 and
  .verification.roots_after_sha256 == $empty and
  .verification.unrelated_tmp_entries_not_in_manifest_preserved == true
' "$TMP_CLEANUP/certificate.json" >/dev/null

DIRECT_FILES="$TMP_CLEANUP/direct-files-before.tsv"
[[ "$(head -n 1 "$DIRECT_FILES")" == \
  $'path\tuid\ttype\tsize\tsha256\tdecision\treason' ]]
[[ "$(wc -l <"$DIRECT_FILES")" == 552 ]]
tail -n +2 "$DIRECT_FILES" | cut -f1 >"$TEMP/direct-file-paths.tsv"
LC_ALL=C sort -u "$TEMP/direct-file-paths.tsv" >"$TEMP/direct-file-paths.sorted"
cmp "$TEMP/direct-file-paths.tsv" "$TEMP/direct-file-paths.sorted"
[[ "$(awk -F '\t' 'NR > 1 {
    if (NF != 7 || $2 != 1003 ||
        ($3 != "regular file" && $3 != "regular empty file") ||
        $4 !~ /^[0-9]+$/ || $5 !~ /^[0-9a-f]{64}$/ || $6 != "delete" ||
        $7 != "task-generated direct /tmp scratch; no accepted evidence dependency")
      bad++
    n++; bytes += $4; types[$3]++
  }
  END {printf "%d\t%d\t%d\t%d\t%d", n, bytes,
    types["regular file"], types["regular empty file"], bad+0}' \
  "$DIRECT_FILES")" == $'551\t225429428\t521\t30\t0' ]]
while IFS=$'\t' read -r target owner kind bytes digest decision reason extra; do
...
grep -q $'^/tmp/task12-keyed-result.json\t' "$DIRECT_FILES"
grep -q $'^/tmp/task10-review-repo-basenames.tsv\t' "$DIRECT_FILES"
[[ ! -s "$TMP_CLEANUP/direct-files-retained.tsv" &&
   ! -s "$TMP_CLEANUP/direct-files-after.txt" ]]
[[ "$(wc -l <"$TMP_CLEANUP/direct-files-reference-scan.tsv")" == 3 ]]
[[ "$(awk -F: '{print $3}' \
  "$TMP_CLEANUP/direct-files-reference-scan.tsv" | LC_ALL=C sort -u)" == \
  $'/tmp/task10-final-selftest.log\n/tmp/task10-h4pedant.log\n/tmp/task10-selftest.log' ]]
[[ "$(awk -F: '$1 != ".agent-files/loop-20260719-192041.log" {bad++}
  END {print bad+0}' "$TMP_CLEANUP/direct-files-reference-scan.tsv")" == 0 ]]
[[ "$(wc -l <"$TMP_CLEANUP/direct-files-reference-scope.tsv")" == 6 ]]
find /tmp -mindepth 1 -maxdepth 1 -user 1003 -printf '%p\n' |
  awk 'tolower($0) ~ /(phase3|task10|task11|task12)/' \
    >"$TEMP/direct-file-residue.tsv"
[[ ! -s "$TEMP/direct-file-residue.tsv" ]]
jq -e \
  --arg manifest "$(sha256sum "$DIRECT_FILES" | cut -d' ' -f1)" \
  --arg references "$(sha256sum \
    "$TMP_CLEANUP/direct-files-reference-scan.tsv" | cut -d' ' -f1)" \
  --arg scope "$(sha256sum \
    "$TMP_CLEANUP/direct-files-reference-scope.tsv" | cut -d' ' -f1)" \
  --arg empty "$(sha256sum "$TMP_CLEANUP/direct-files-after.txt" |
    cut -d' ' -f1)" \
  --arg retained "$(sha256sum "$TMP_CLEANUP/direct-files-retained.tsv" |
    cut -d' ' -f1)" \
  --arg script "$(sha256sum "$TMP_CLEANUP/delete-classified-files.sh" |
    cut -d' ' -f1)" '
  .direct_files.scope ==
    "direct /tmp entries owned by uid 1003 whose basename contains phase3 or task10/task11/task12" and
  .direct_files.targets == 551 and
  .direct_files.apparent_bytes == 225429428 and
  .direct_files.types == {regular_files:521,regular_empty_files:30} and
  .direct_files.manifest_sha256 == $manifest and
  .direct_files.manifest_columns ==
    ["path","uid","type","size","sha256","decision","reason"] and
  .direct_files.dependency_validation.repository_reference_matches == 3 and
  .direct_files.dependency_validation.accepted_evidence_dependency_matches == 0 and
  .direct_files.dependency_validation.reference_scan_sha256 == $references and
  .direct_files.dependency_validation.reference_scope_sha256 == $scope and
  .direct_files.dependency_validation.historical_log_only_paths ==
    ["/tmp/task10-final-selftest.log","/tmp/task10-h4pedant.log",
     "/tmp/task10-selftest.log"] and
  .direct_files.dependency_validation.sole_dependency_found == false and
  .direct_files.classification.delete == 551 and
  .direct_files.classification.retain == 0 and
  .direct_files.classification.retained_manifest_sha256 == $retained and
  .direct_files.classification.unrelated_entries_preserved == true and
  .direct_files.deletion.deleted == 551 and
  .direct_files.deletion.path_validation_before_deletion == true and
  .direct_files.deletion.direct_children_only == true and
  .direct_files.deletion.canonical_path_required == true and
  .direct_files.deletion.symlink_targets_rejected == true and
  .direct_files.deletion.owner_type_size_sha256_rechecked == true and
  .direct_files.deletion.safe_delete_script_sha256 == $script and
  .direct_files.deletion.audit_transient_removed ==
    ["/tmp/task12-cleanup-audit-initial.tsv"] and
  .direct_files.verification.matching_entries_remaining == 0 and
  .direct_files.verification.entries_after_sha256 == $empty
' "$TMP_CLEANUP/certificate.json" >/dev/null

jq -e --arg tmp "$(sha256sum "$TMP_CLEANUP/certificate.json" |
    cut -d' ' -f1)" \
  --arg roots "$(sha256sum "$TMP_CLEANUP/roots-before.tsv" |
    cut -d' ' -f1)" \
  --arg files "$(sha256sum "$TMP_CLEANUP/direct-files-before.tsv" |
    cut -d' ' -f1)" '
  .schema == "hh-task12-cleanup-v6" and .status == "complete" and
  .removed_roots == [
  ...
  .tmp_cleanup_certificate_sha256 == $tmp and
  .tmp_cleanup_roots_manifest_sha256 == $roots and
  .tmp_cleanup_roots == 224 and
  .tmp_cleanup_files_manifest_sha256 == $files and
  .tmp_cleanup_files == 551 and
...
cmp "$K/source-sample-certificate.json" "$SAMPLE/sample-certificate.json"
...
jq -e '
  .schema == "hh-task12-paired-sample-v4" and
  .new_slice_classification ==
    "exact full schedule key, order independent" and
  .anchor_exclusive_counting ==
    "slice-attributed; one goal may contribute to multiple slice/filter sums" and
  [.new_slices[] |
    [.index,.anchor_exclusive,.anchor_exclusive_reconstructed]] ==
    [[17,77,65],[18,30,29],[19,4,4],[20,19,1],
     [21,9,9],[22,60,51],[23,5,3],[24,42,38]] and
  [.new_filter_aggregates[] |
    [.filter,.anchor_exclusive,.anchor_exclusive_reconstructed]] ==
    [["mash",47,41],["mepo",69,60],["mesh",130,99]]
' "$SAMPLE/result.json" >/dev/null
jq -n --slurpfile old "$SAMPLE/s30v3-sample.jsonl" \
  --slurpfile new "$SAMPLE/s30v5-sample.jsonl" \
...
jq -e --arg sample "$(sha256sum "$SAMPLE/SHA256SUMS" | cut -d' ' -f1)" \
  ...
  --arg tmp "$(sha256sum "$TMP_CLEANUP/certificate.json" |
    cut -d' ' -f1)" '
  .schema == "hh-task12-final-v6" and .status == "complete" and
