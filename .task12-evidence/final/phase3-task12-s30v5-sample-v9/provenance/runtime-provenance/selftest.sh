...
# Slice completion order is not semantic.  Reverse all 24 slice rows in every
# sampled goal and require both maintained and frozen folds to accept the exact
# same result.  Then require coherent missing, duplicate, and wrong full keys to
# fail closed before a result can be produced.
old="$FINAL/sample/s30v3-sample.jsonl"
schedule="$FINAL/sample/schedule.tsv"
jq -c '.slices |= reverse' "$FINAL/sample/s30v5-sample.jsonl" \
  >"$TEMP/permuted-slices.jsonl"
for fold in "$TOOLS/paired-result.jq" "$FINAL/verifier/paired-result.jq"; do
  jq -n --slurpfile old "$old" \
    --slurpfile new "$TEMP/permuted-slices.jsonl" \
    --rawfile schedule "$schedule" -f "$fold" | jq -S . \
    >"$TEMP/permuted-result.json"
  cmp "$TEMP/permuted-result.json" "$FINAL/sample/result.json"
done

expect_bad_schedule_key() {
  local label=$1 journal=$2
  if jq -n --slurpfile old "$old" --slurpfile new "$journal" \
      --rawfile schedule "$schedule" -f "$TOOLS/paired-result.jq" \
      >/dev/null 2>&1; then
    echo "$label schedule-key mutation unexpectedly folded" >&2
    exit 1
  fi
}
jq -c 'if input_line_number == 1 then
  .slices[0].slice.nfacts += 1 else . end' \
  "$FINAL/sample/s30v5-sample.jsonl" >"$TEMP/wrong-key.jsonl"
expect_bad_schedule_key wrong "$TEMP/wrong-key.jsonl"
jq -c 'if input_line_number == 1 then
  .slices[0].slice = .slices[1].slice else . end' \
  "$FINAL/sample/s30v5-sample.jsonl" >"$TEMP/duplicate-key.jsonl"
expect_bad_schedule_key duplicate "$TEMP/duplicate-key.jsonl"
jq -c 'if input_line_number == 1 then
  .slices = .slices[0:23] else . end' \
  "$FINAL/sample/s30v5-sample.jsonl" >"$TEMP/missing-key.jsonl"
expect_bad_schedule_key missing "$TEMP/missing-key.jsonl"
...
file_safe="$FINAL/provenance/tmp-cleanup/delete-classified-files.sh"
file_fixture="/tmp/task12-direct-file-selftest-$$"
file_manifest="$TEMP/direct-file-manifest.tsv"
printf 'selftest\n' >"$file_fixture"
printf 'path\tuid\ttype\tsize\tsha256\tdecision\treason\n' >"$file_manifest"
printf '%s\t%s\tregular file\t%s\t%s\tdelete\tselftest\n' \
  "$file_fixture" "$(id -u)" "$(stat -c %s "$file_fixture")" \
  "$(sha256sum "$file_fixture" | cut -d' ' -f1)" >>"$file_manifest"
"$file_safe" "$file_manifest"
[[ ! -e "$file_fixture" ]]

file_escape="/tmp/task12-direct-file-selftest-escape-$$"
printf 'selftest\n' >"$file_escape"
printf 'path\tuid\ttype\tsize\tsha256\tdecision\treason\n' >"$file_manifest"
printf '/tmp/../tmp/%s\t%s\tregular file\t%s\t%s\tdelete\tselftest\n' \
  "${file_escape#/tmp/}" "$(id -u)" "$(stat -c %s "$file_escape")" \
  "$(sha256sum "$file_escape" | cut -d' ' -f1)" >>"$file_manifest"
if "$file_safe" "$file_manifest" >/dev/null 2>&1; then
  echo "non-direct file-cleanup path unexpectedly accepted" >&2
  exit 1
fi
[[ -f "$file_escape" ]]
rm -- "$file_escape"

file_link="/tmp/task12-direct-file-selftest-link-$$"
ln -s "$TEMP" "$file_link"
printf 'path\tuid\ttype\tsize\tsha256\tdecision\treason\n' >"$file_manifest"
printf '%s\t%s\tregular file\t0\t%s\tdelete\tselftest\n' \
  "$file_link" "$(id -u)" \
  'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855' \
  >>"$file_manifest"
if "$file_safe" "$file_manifest" >/dev/null 2>&1; then
  echo "symlink file-cleanup target unexpectedly accepted" >&2
  exit 1
fi
[[ -L "$file_link" ]]
rm -- "$file_link"
