...
local
  ...
  val (_,_,_,_,_,RING_POLY_CONV) =
    SEMIRING_NORMALIZERS_CONV
     RING_POLY_UNIVERSAL_CONV_pth RING_POLY_UNIVERSAL_CONV_sth
     (is_ringconst,
...
val RING_POLY_UNIVERSAL_CONV =
    GEN_REWRITE_CONV ONCE_DEPTH_CONV
      [RING_POLY_UNIVERSAL_CONV_ith, GSYM RING_OF_INT_OF_NUM]
    THENC RING_POLY_CONV
end;

(* 2. RING_INTEGRAL_DOMAIN_UNIVERSAL and ring_ring_cofactors_universal *)
local
  val rule1 = PART_MATCH (lhand o lhand) RING_INTEGRAL_DOMAIN_UNIVERSAL_neth_g;
  val conv1 =
    RAND_CONV INT_SUB_CONV THENC
    GEN_REWRITE_CONV TRY_CONV [RING_INTEGRAL_DOMAIN_UNIVERSAL_neth_h];
  fun RING_EQ_CONV tm =
    PART_MATCH lhand RING_INTEGRAL_DOMAIN_UNIVERSAL_neth_b tm
    handle HOL_ERR _ =>
      PART_MATCH lhand RING_INTEGRAL_DOMAIN_UNIVERSAL_neth_l tm
      handle HOL_ERR _ =>
        PART_MATCH lhand RING_INTEGRAL_DOMAIN_UNIVERSAL_neth_r tm
        handle HOL_ERR _ =>
...
in
  fun RING_INTEGRAL_DOMAIN_UNIVERSAL tm =
    let val th = decorule tm in
      EQ_MP (SYM th) (basic_rule(rand(concl th)))
    end
  val ring_ring_cofactors_universal = idealconv
end;
...
fun RING_MONOMORPHIC_IMAGE_RULE hth = let
    val ([pth_eq, pth_asm,
      ...
      splitlist CONJ_PAIR (MATCH_MP RING_MONOMORPHIC_IMAGE_RULE_THM hth)
...
in
  fun replay_implication avs ant con ctms =
    let
      ...
      val zth =
        end_itlist (fn th1 => fn th2 =>
          MATCH_MP RING_WORD_UNIVERSAL_dth (CONJ th1 th2)) zths;
      val eth =
...
local
  ...
  val left_exists_rule = GEN_REWRITE_RULE TRY_CONV [LEFT_FORALL_IMP_THM];
  val or_elim_rule     = GEN_REWRITE_RULE TRY_CONV [RING_RING_WORD_pth];
in
...
local
  val ddj_conv =
    GEN_REWRITE_CONV (RAND_CONV o DEPTH_CONV) [RING_RING_HORN_pth1] THENC
    GEN_REWRITE_CONV TRY_CONV  [RING_RING_HORN_pth2]
in
...
in
  fun RING_RING_CORE tm = let
      ...
      val th1 = INST [ptm |-> concl th, qtm |-> tm] RING_RING_CORE_pth;
