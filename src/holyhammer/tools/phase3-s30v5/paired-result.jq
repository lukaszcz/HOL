  ...
  [$specs[16:24][] as $s | ($s|skey) as $key |
    {index:$s.index,prover:$s.prover,filter:$s.filter,
     ...
     anchor_exclusive:([$n[] | select(
       any(.slices[16:24][];(.slice|skey)==$key and .szs=="Theorem") and
       all(.slices[0:16][];.szs!="Theorem"))]|length),
     anchor_exclusive_reconstructed:([$n[] | select(recon and
       any(.slices[16:24][];(.slice|skey)==$key and .szs=="Theorem") and
       all(.slices[0:16][];.szs!="Theorem"))]|length)}] as $slices |
  {schema:"hh-task12-paired-sample-v3",status:"complete",
